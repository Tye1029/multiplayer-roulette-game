...
exports.handler = async (event) => {
  ...
  if (!/^[A-Za-z0-9]{8,64}$/.test(visitorKey)) {
    return json(headers, 400, { ok: false, error: "Save your Torn API key before playing Arcade Games." });
  }

  try {
    const visitorProfile = await tornFetch(`/user/?selections=profile&key=${encodeURIComponent(visitorKey)}&comment=xans-arcade-games`);
    ...
    const result = await arcadePlayGame(user, { ...body, lastWithdrawal: body.lastWithdrawal });
    ...
    console.error("[arcade-action] Error:", error.message || error);
    return json(headers, 500, { ok: false, error: error.message || "Unable to update Arcade Games." });
  }
};
...
async function tornFetch(path) {
  const response = await fetch(`${TORN_API_BASE}${path}`, {
    headers: { "User-Agent": "CowBoyCookie-Xan-Arcade-Games/1.0" }
  });
