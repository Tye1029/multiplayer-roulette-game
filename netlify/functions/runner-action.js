...
exports.handler = async (event) => {
  ...
  if (!/^[A-Za-z0-9]{8,64}$/.test(visitorKey)) {
    return json(headers, 400, { ok: false, error: "Save your Torn API key before playing Street Runner." });
  }

  const action = String(body.action || "get").toLowerCase();

  try {
    const visitorProfile = await tornFetch(`/user/?selections=profile&key=${encodeURIComponent(visitorKey)}&comment=xans-street-runner`);
    ...
    let result;
    if (action === "get") {
      result = await runnerGetGame(user);
    } else if (action === "start") {
      result = await runnerStartGame(user, { wager: body.wager, lastWithdrawal: body.lastWithdrawal });
    } else if (action === "choose") {
      result = await runnerChooseDirection(user, {
        blocks: body.blocks,
        direction: body.direction,
        lastWithdrawal: body.lastWithdrawal
      });
    } else if (action === "cashout") {
      result = await runnerCashOut(user, { lastWithdrawal: body.lastWithdrawal });
    } else {
      return json(headers, 400, { ok: false, error: "Unknown Street Runner action." });
    }
    ...
    console.error("[runner-action] Error:", error.message || error);
    return json(headers, 500, { ok: false, error: error.message || "Unable to update Street Runner." });
  }
};
...
async function tornFetch(path) {
  const response = await fetch(`${TORN_API_BASE}${path}`, {
    headers: { "User-Agent": "CowBoyCookie-Xan-Street-Runner/1.0" }
  });
