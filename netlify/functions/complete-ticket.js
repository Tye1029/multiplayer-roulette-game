const { initBlobs, completeTicket, resolveSiteUser } = require("./_data");
...
exports.handler = async (event) => {
  ...
  const ticketToken = String(body.ticketToken || body.token || "");
  if (!ticketId) return json(headers, 400, { ok: false, error: "Missing ticket ID." });

  try {
    const visitorProfile = await tornFetch(`/user/?selections=profile&key=${encodeURIComponent(visitorKey)}&comment=xans-complete-ticket`);
    ...
    const { record, ticket, bet, duplicate } = await completeTicket(siteUser, { ticketId, ticketToken, lastWithdrawal: body.lastWithdrawal });

    console.log(`[complete-ticket] ${visitor.name} [${visitor.id}] ticket=${ticket.ticketId} cost=${ticket.ticketCost} won=${ticket.winAmount} balance=${record.currentBalance}${duplicate ? " duplicate" : ""}`);
    ...
    console.error("[complete-ticket] Error:", error.message || error);
    return json(headers, 500, { ok: false, error: error.message || "Unable to complete ticket." });
  }
};
