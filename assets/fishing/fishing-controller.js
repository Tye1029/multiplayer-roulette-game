(function fishingControllerBootstrap(global){
  ...
  const VERSION="fishing-controller-v4";
  ...
  class FishingSceneController{
    ...
    drawWater(now){
      ...
      const phase=this.reducedMotion?0:now*.00065;
      const gradient=ctx.createLinearGradient(0,horizon,0,h);gradient.addColorStop(0,"rgba(86,238,244,.02)");gradient.addColorStop(.55,"rgba(18,171,205,.04)");gradient.addColorStop(1,"rgba(0,83,128,.06)");ctx.fillStyle=gradient;ctx.fillRect(0,horizon,w,h-horizon);
      ...
      for(let row=0;row<18;row++){
        ...
        ctx.strokeStyle=`rgba(${row%3===0?"221,254,255":"92,231,235"},${.043+(row%4)*.01})`;ctx.lineWidth=row%4===0?1.15:.65;ctx.stroke();
      }
      for(let band=0;band<3;band++){
        ...
        ctx.strokeStyle="rgba(214,252,255,.03)";ctx.lineWidth=3.5+band;ctx.stroke();
      }
