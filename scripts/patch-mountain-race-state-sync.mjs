...
if (!client.includes(clientMarker)) {
  ...
  client = replaceRequired(
    ...
    const sameMeaningfulState = Boolean(previousGame?.gameId === game.gameId
      && runtime.renderKey === nextRenderKey
      && !hadPending
      && !resolvingPending);
    const confirmedInputAt = game.mountainraceState?.me?.lastInput?.at || '';
    ...
    'monotonic adoption and action lock'
  );
  ...
  client = replaceRequired(
    ...
    'background state-event separation'
  );
...
if (!client.includes("runtime.game?.status !== 'complete' || !runtime.resultRevealReady")) throw new Error('Summit Sprint result card does not wait for the final climb.');
await writeFile(clientUrl, client);

let html = await readFile(indexUrl, 'utf8');

if (!html.includes('function mountainRaceAcceptSnapshot(game)')) {
  const guardBlock = `// MOUNTAIN_RACE_SHARED_SNAPSHOT_GUARD_START
...
if (!html.includes('const acceptedGame = data.game && mountainRaceAcceptSnapshot(data.game)')) throw new Error('Action responses can still regress Summit Sprint.');
if (!html.includes('game?.mountainraceState||{}')) throw new Error('Network Bot ordering ignores Summit Sprint state revisions.');
if (!html.includes('mountain-race-multiplayer.js?v=1&gameplay=3&load=2&sync=1')) throw new Error('Fresh state-sync client cache boundary is missing.');
await writeFile(indexUrl, html);

console.log('Synchronized Summit Sprint input and visuals: one tap remains locked until its own response, stale GET/action/bot snapshots are rejected, progress cannot move backward, and the final climb is shown before results.');
