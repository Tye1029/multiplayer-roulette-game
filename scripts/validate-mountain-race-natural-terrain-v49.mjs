...
for (const document of [html, preview]) {
  assert(document.includes(natural56 ? 'visual=56' : route55 ? 'visual=55' : grounded54 ? 'visual=54' : camera53 ? 'visual=53' : winner52 ? 'visual=52' : shared51 ? 'visual=51' : summit50 ? 'visual=50' : 'visual=49'), 'V49/V56 cache boundary is missing');
...
assert(runtime.includes(natural56 ? 'currentIndex + 3' : route55 ? 'currentIndex + 4' : grounded54 ? 'currentIndex + 7' : 'currentIndex + 3'), 'multiplayer renderer no longer keeps nearby ledges');
...
assert(runtime.includes(natural56 ? 'Math.max(0, cameraIndex) * 74' : route55 ? 'Math.max(0, cameraIndex - 1) * 60' : grounded54 ? 'Math.max(0, cameraIndex - 1) * 42' : 'Math.max(0, cameraIndex - 1) * 84'), 'fixed camera framing changed');
