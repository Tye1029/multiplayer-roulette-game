...
const checks = [
  ...
  ['authoritative render stage increase triggers the correct cue directly', client.includes('function safeCrackerRenderWithAuthoritativeCorrectCue(game)') && client.includes('const previousStage = sameGame ? Number(previousGame?.safecrackerState?.me?.stage || 0) : 0;') && client.includes('const nextStage = Number(game?.safecrackerState?.me?.stage || 0);') && client.includes('if (sameGame && nextStage > previousStage)') && client.includes('safeCrackerPlayAuthoritativeCorrectCue(game, nextStage);')],
...
console.log('Safe Cracker click-cue validation passed: the dial uses dry non-musical metal ratchets, incorrect guesses use a clear three-part rejected-lock sequence, correct guesses fire from authoritative render-stage progress, and protected gameplay boundaries remain intact.');
