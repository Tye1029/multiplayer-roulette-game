...
const checks = [
  ...
  ['dry metal impact uses filtered noise without an oscillator', client.includes('function safeCrackerPlayDryMetalImpact(options = {})') && client.includes("primary.type = 'bandpass';") && client.includes("secondary.type = 'bandpass';") && !dialSource.includes('safeCrackerPlayClickTransient(') && !dialSource.includes('createOscillator')],
  ['dial has separate tooth and catch impacts', dialSource.includes('const tooth = safeCrackerPlayDryMetalImpact({') && dialSource.includes('const catchClick = safeCrackerPlayDryMetalImpact({') && dialSource.includes('delay: 0.018')],
  ['incorrect-number cue remains obvious and distinct', client.includes('function safeCrackerPlayIncorrectRejectCue(tier)') && client.includes('const metalStop = safeCrackerPlayDryMetalImpact({') && client.includes('const internalKnock = safeCrackerPlayClickTransient({') && client.includes('const rejectClack = safeCrackerPlayDryMetalImpact({')],
  ...
  ['incorrect feedback still uses both generated and recorded rejection sounds', client.includes('const cuePlayed = safeCrackerPlayIncorrectRejectCue(tier);') && client.includes("safeCrackerPlayRecordedSound('incorrect'")],
...
console.log('Safe Cracker click-cue validation passed: the dial uses dry non-musical metal ratchets, incorrect guesses retain a clear rejected-lock sequence, correct guesses fire directly from authoritative stage progress, and protected gameplay boundaries remain intact.');
