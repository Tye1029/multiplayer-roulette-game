...
if (!/safe-cracker\.css\?[^"'\s]*&dial=7[^"'\s]*&layout=7/.test(html)) {
  throw new Error('Safe Cracker dial-layout validation failed: the final stylesheet cache key is missing layout=7.');
}
if (!/safe-cracker\.js\?[^"'\s]*&dial=7[^"'\s]*&layout=7/.test(html)) {
  throw new Error('Safe Cracker dial-layout validation failed: the final runtime cache key is missing layout=7.');
}
...
console.log('Safe Cracker dial-layout v2 validation passed beneath the final fixed-label confirmation refinement: pointer layout, centered step symbols, brighter base button, current cache busting, V16 retention, and protected Roulette boundaries remain intact.');
